-- Kerigma EAD 2.0 - Migração base EAD + Bucket + Seeds
-- 1) Tabela de trilhas de formação
CREATE TABLE IF NOT EXISTS public.trilhas_formacao (
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_trilhas_formacao_updated_at'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_modulos_curso_updated_at'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_licoes_updated_at'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_licao_anexos_updated_at'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_progresso_licoes_updated_at'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_gamificacao_recompensas_updated_at'
  ) THEN
...
-- 9) Bucket de storage para anexos EAD
INSERT INTO storage.buckets (id, name, public)
VALUES ('ead-anexos', 'ead-anexos', false)
ON CONFLICT (id) DO NOTHING;

-- Políticas de acesso no bucket 'ead-anexos'
DO $$ BEGIN
  IF NOT EXISTS (
...
-- Observação importante: a convenção de caminho no storage será:
-- <curso_id>/<licao_id>/<arquivo>
-- Ex.: 2f3e...-uuid/9a1b...-uuid/slides.pdf

-- 10) Seed inicial - Trilha DNA, cursos e lições
...
-- Triggers de updated_at para cursos se ainda não existirem (caso não exista)
DO $$ BEGIN
  IF NOT EXISTS (
