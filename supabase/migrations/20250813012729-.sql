...
-- 1) Células ativas (apenas se ainda não houver)
DO $$
DECLARE v_count int;
BEGIN
  ...
  IF v_count = 0 THEN
    ...
    ('Célula Famílias', 'Renascença', 'Rua das Famílias, 77', 'Terça', '19:30:00', 'São Luís', '65000-002', 'Verde', 'Famílias da região'),
    ('Célula Infantil', 'Turu', 'Rua das Crianças, 12', 'Sábado', '16:00:00', 'São Luís', '65000-003', 'Verde', 'Crianças e pais'),
...
BEGIN
  ...
  IF v_cal_id IS NULL THEN
    INSERT INTO public.calendarios (nome, cor, tipo, visivel_para_todos, descricao)
    VALUES ('Eventos da Igreja', '#2563eb', 'organizacional', true, 'Agenda geral de eventos')
