...
create policy if not exists "Participants can view their agendamentos"
...
create policy if not exists "Users can view own reservas or admins"
  on public.patrimonio_reservas
  for select
  using (solicitante_id = public.get_current_person_id() or public.is_admin());

create policy if not exists "Users can create own reservas"
  on public.patrimonio_reservas
  for insert
  with check (solicitante_id = public.get_current_person_id());

create policy if not exists "Users can update own reservas or admins"
  on public.patrimonio_reservas
  for update
  using (solicitante_id = public.get_current_person_id() or public.is_admin())
  with check (solicitante_id = public.get_current_person_id() or public.is_admin());

create policy if not exists "Users can delete own reservas or admins"
  on public.patrimonio_reservas
  for delete
  using (solicitante_id = public.get_current_person_id() or public.is_admin());
