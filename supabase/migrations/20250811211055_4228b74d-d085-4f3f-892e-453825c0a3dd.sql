...
-- RLS: Admins can manage all user_permissions
...
-- RLS: Users can view their own user permissions
create policy if not exists "Users can view own user permissions"
  on public.user_permissions
  as permissive
  for select
  to authenticated
...
create trigger if not exists trg_abac_rules_updated
...
create policy if not exists "Admins can manage ABAC rules"
...
create policy if not exists "Read active ABAC rules"
  on public.abac_rules
  as permissive
  for select
  to authenticated
