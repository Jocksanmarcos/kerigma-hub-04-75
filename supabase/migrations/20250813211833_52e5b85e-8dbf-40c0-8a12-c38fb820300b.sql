-- Função para atualizar progresso quando um capítulo é lido
...
DECLARE
  ...
  pontos_ganhos INTEGER := 10; -- 10 pontos por capítulo lido
BEGIN
  -- Buscar o pessoa_id baseado no user_id autenticado
  ...
  -- Atualizar ou criar progresso na tabela estudo_biblico_progresso
  ...
  VALUES (
    pessoa_id_usuario,
    pontos_ganhos,
    1, -- 1 versículo lido (será atualizado pela função de agregação)
    1, -- sequência de dias (será calculada)
    NEW.id,
    '{}'::jsonb
  )
...
-- Função para marcar capítulo como lido (para gamificação)
...
DECLARE
  ...
  pontos_capitulo INTEGER := 50; -- 50 pontos por capítulo completo
BEGIN
  -- Buscar o pessoa_id baseado no user_id autenticado
  SELECT id INTO pessoa_id_usuario
  FROM public.pessoas 
  ...
  -- Buscar o primeiro versículo do capítulo
