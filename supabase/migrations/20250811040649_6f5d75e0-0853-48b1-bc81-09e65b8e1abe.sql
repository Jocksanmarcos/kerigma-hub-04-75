-- Unified Ministries: invites + codes (retry)
...
-- 4) Seed codes for existing teams
...
-- 9) Enable RLS + policy for convites_ministerio
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'DROP POLICY "Leaders manage convites_ministerio" ON public.convites_ministerio';
  END IF;
END$$;
