-- Full idempotent patch for EAD setup with trilhas_formacao, ensuring required columns incl. "nome" and "slug" before seeding

-- 1) Patch trilhas_formacao columns
DO $$ BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema='public' AND table_name='trilhas_formacao' AND column_name='nome'
  ) THEN
    ALTER TABLE public.trilhas_formacao ADD COLUMN nome text;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
DO $$ BEGIN
  -- Enable only if table exists
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema='public' AND table_name='trilhas_formacao'
  ) THEN
    EXECUTE 'ALTER TABLE public.trilhas_formacao ENABLE ROW LEVEL SECURITY';
  END IF;
END $$;

-- Index and policies for trilhas_formacao
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema='public' AND table_name='trilhas_formacao'
...
-- 6) Seeds (now safe with nome and slug ready)
